...
def get_data(ftype: str, validation_percent=0.2):
    datafiles = {"test":"test.csv", "train":"train.csv"}
    assert ftype in datafiles.keys(), f"choose ftype from {datafiles.keys()}"

    raw_df = pd.read_csv(Path.cwd().joinpath("data",datafiles[ftype]),
    ...
    ys_train = train_df.iloc[:,0].values.astype('float32') / 255.0
    xs_validation = validation_df.iloc[:,1:].values
    ys_validation = validation_df.iloc[:,0].values

    return xs_train, ys_train, xs_validation, ys_validation
...
def train(log_reg, xs_train, ys_train, xs_val, ys_val,
          batch_size = 64, epochs = 200, learning_rate = 0.01,
          plot=True):
...
def main():
    xs_train, ys_train, xs_val, ys_val = get_data("train")
...
def main2():
    x_train, y_train, x_val, y_val = get_data("train")
